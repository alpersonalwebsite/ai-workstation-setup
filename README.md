...
| [`claude/settings.example.json`](claude/settings.example.json) | Wires the hooks and status line, denies reads and edits of secret files (`//**/.env*`, `~/.ssh/**`, `~/.aws/credentials` and kin, the `//` anchor being load-bearing); copy to `~/.claude/settings.json` or merge into yours. Sets no `attribution` key, so the AI-assistance trailer is left to your own policy, see docs |
