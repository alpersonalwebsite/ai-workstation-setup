...
| [`claude/settings.example.json`](claude/settings.example.json) | Wires the hooks and status line, denies reads and edits of secret files (`//**/.env*`, `~/.ssh/**`, `~/.aws/credentials` and kin, the `//` anchor being load-bearing), and strips AI attribution from commits and PRs; copy to `~/.claude/settings.json` or merge into yours |
